...
const ElectrodeCombinationsVisualizer = () => {
  ...
  // Generate 20 dots positioned evenly inside the circle
  useEffect(() => {
    const newDots = [];
    
    // Parameters for spiral distribution
    const numRings = 3;
    const dotsPerRing = [1, 6, 13]; // Total 20 dots
    let dotIndex = 1;
    ...
    // Generate rings of dots
    for (let ring = 1; ring < numRings; ring++) {
      const ringRadius = (RADIUS * 0.8) * (ring / numRings);
      const numDotsInRing = dotsPerRing[ring];
      
      for (let i = 0; i < numDotsInRing; i++) {
        const angle = (i / numDotsInRing) * 2 * Math.PI;
